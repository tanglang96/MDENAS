...
network_path = {'gpu': 'mobile_config/gpu.config', 'cpu': 'mobile_config/cpu.config'}
...
def proxyless_network(structure, genotypes, n_classes=1000, bn_param=(0.1, 1e-3), dropout_rate=0):
    net_config_json = json.load(open(network_path[structure], 'r'))
    net_config_json['classifier']['out_features'] = n_classes
    net_config_json['classifier']['dropout_rate'] = dropout_rate
    if len(genotypes) > 0:
        non_zero_index = []
        for block_index, block in enumerate(net_config_json['blocks']):
            if block['mobile_inverted_conv']['name'] == 'MBInvertedConvLayer':
                non_zero_index.append(block_index)
        print(non_zero_index)
        print(genotypes)
        assert len(genotypes) == len(non_zero_index), "The length of genotypes and blocks should be same!"
        for index, genotype in enumerate(genotypes):
            net_config_json['blocks'][non_zero_index[index]]['mobile_inverted_conv']['kernel_size'] = genotype[0]
            net_config_json['blocks'][non_zero_index[index]]['mobile_inverted_conv']['expand_ratio'] = genotype[1]
    net_config_json['classifier']['out_features'] = n_classes
    net_config_json['classifier']['dropout_rate'] = dropout_rate
    net = ProxylessNASNets.build_from_config(net_config_json)
    net.set_bn_param(momentum=bn_param[0], eps=bn_param[1])
