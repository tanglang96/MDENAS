...
class SearchCell(nn.Module):
    ...
    def __init__(self, n_nodes, C_pp, C_p, C, reduction_p, reduction):
        ...
        for i in range(self.n_nodes):
            ...
            for j in range(2 + i):  # include 2 input nodes
                ...
                op = ops.MixedOp(C, stride)  # 搜索的时候都是混合操作
    ...
    def forward(self, s0, s1, w_dag):
        ...
        states = [s0, s1]
        for edges, w_list in zip(self.dag, w_dag):  # 遍历每个内部节点
            s_cur = sum(edges[i](s, w) for i, (s, w) in enumerate(zip(states, w_list)))  # 将输入到节点的所有边的输出加起来作为该节点的输入
            states.append(s_cur)
